...
function assignObjectsN(...objn) {
    return object = Object.assign(...objn);
}
...
function assignObjects2(...objn) {
    ...
    for (let index = 0; index < objn.length; index++) {
        for (var key in objn[index]) {
            if (objn[index].hasOwnProperty(key)) {
                obj[key] = objn[index][key];
            }
        }
    }
    console.log(obj)
...
assignObjects2({ x: 10, y: 20 }, { z: 30, d: 12 }, { a: 15 }, { b: 35 }, { c: 45 });
